...
// Which engine plays on a TELEVISION. "auto" resolves to the native Android player, which is
// the one that actually renders a picture on Fire OS — the embedded (WebView + hls.js) engine
// is where the P2P mesh lives, but on a Fire Stick it currently plays audio without video, so
// it is NOT the default. Phones and desktop are unaffected: they use the classic layout's
// player, which has always been the WebView one (and therefore has sharing).
